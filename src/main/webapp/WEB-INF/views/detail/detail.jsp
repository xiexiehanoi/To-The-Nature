    ...
    <div id="cont_inner">
        <div class="sub_layout layout">
            <div class="camp_info_box">
                ...
                <!-- 테이블 끝 -->
                    ...
                    <div class="image_grid">
                    	<c:forEach var="imageUrl" items="${fn:split(dto.imageUrl, ',')}" varStatus="loop">
                    		<img src="${imageUrl.trim()}" alt="캠핑장 이미지" class="${loop.index + 1 > 6 ? 'hidden-image' : ''}">
                        </c:forEach>
                    </div>
            ...
            <div class="camping_information_group">
				<h3 class="camping_info"><i class="bi bi-info-circle-fill"></i>&nbsp;캠핑장 소개</h3>
				<p class="camping_intro">
				...
				<h3 class="camping_info"><i class="bi bi-info-circle-fill"></i>&nbsp;캠핑장 시설정보</h3>
        ...
        <c:forEach var="sbrcl" items="${fn:split(dto.sbrsCl, ',')}">
            <c:choose>
                <c:when test="${sbrcl == '운동시설'}">
                    <li><i class="bi bi-dumbbell"><span>${sbrcl}</span></i></li>
                </c:when>
                <c:when test="${sbrcl == '전기'}">
                    <li><i class="bi bi-lightning"><span>${sbrcl}</span></i></li>
                </c:when>
                <c:when test="${sbrcl == '무선인터넷'}">
                    <li><i class="bi bi-wifi"><span>${sbrcl}</span></i></li>
                </c:when>
                <c:when test="${sbrcl == '장작판매'}">
                    <li><i class="bi bi-tree"><span>${sbrcl}</span></i></li>
                </c:when>
                <c:when test="${sbrcl == '온수'}">
                    <li><i class="bi bi-hot"><span>${sbrcl}</span></i></li>
                </c:when>
                <c:when test="${sbrcl == '트렘폴린'}">
                    <li><i class="bi bi-trampoline"><span>${sbrcl}</span></i></li>
                </c:when>
                <c:when test="${sbrcl == '물놀이장'}">
                    <li><i class="bi bi-droplet"><span>${sbrcl}</span></i></li>
                </c:when>
                <c:when test="${sbrcl == '마트.편의점'}">
                    <li><i class="bi bi-shop"><span>${sbrcl}</span></i></li>
                </c:when>
                <c:when test="${sbrcl == '놀이터'}">
                    <li><i class="bi bi-trophy"><span>${sbrcl}</span></i></li>
                </c:when>
                <c:when test="${sbrcl == '산책로'}">
                    <li><i class="bi bi-tree"><span>${sbrcl}</span></i></li>
                </c:when>
                <c:when test="${sbrcl == '운동장'}">
                    <li><i class="bi bi-basketball"><span>${sbrcl}</span></i></li>
                </c:when>
            </c:choose>
        </c:forEach>
    </ul>
<%-- 			        <c:if test="${sbrcl == '운동시설'}">
			            <i class="fa-solid fa-dumbbell"></i><br>
			            <span>${sbrcl}</span>
			        </c:if>
			        <c:if test="${sbrcl == '전기'}">
			            <i class="fa-regular fa-bolt"></i><br>
			            <span>${sbrcl}</span>
			        </c:if>
			        <c:if test="${sbrcl == '무선인터넷'}">
			            <i class="fa-solid fa-wifi"></i><br>
			            <span>${sbrcl}</span>
			        </c:if>
			        <c:if test="${sbrcl == '장작판매'}">
			            <i class="bi bi-tree"></i><br>
			            <span>${sbrcl}</span>
			        </c:if>
			        <c:if test="${sbrcl == '온수'}">
			            <i class="bi bi-hot"></i><br>
			            <span>${sbrcl}</span>
			        </c:if>
			        <c:if test="${sbrcl == '트렘폴린'}">
			            <i class="bi bi-trampoline"></i><br>
			            <span>${sbrcl}</span>
			        </c:if>
			        <c:if test="${sbrcl == '물놀이장'}">
			            <i class="bi bi-droplet"></i><br>
			            <span>${sbrcl}</span>
			        </c:if>
			        <c:if test="${sbrcl == '마트.편의점'}">
