...
<script>
        function check() {
            ...
            var password = "${sessionScope.userpw}";

            if (checkbox.checked && document.getElementById('username').value === username && document.getElementById('userpw').value === password) {
...
4.회원탈퇴 신청 후 신청 취소를 원하시면 5일 이내에 고객센터로 요청해주시기 바랍니다.<br> 5일 이후에는 회원탈퇴가 완료되어 해당 계정에 대한 모든 정보는 삭제되며 복원이 불가능 합니다. <br>또한 기존의 계정정보 보호를 위해 탈퇴 신청 후 5일 동안은 재가입이 제한됩니다.<br><br>
